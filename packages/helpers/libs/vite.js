...
import operators, { OPERATOR_SOURCE_FOLDER, generateAssetsJson } from '@aklive2d/operator'
...
export const copyShowcaseData = (name, {
  dataDir,
  publicAssetsDir
}) => {
  ...
  const q = [
    {
      ...
      fn: file.symlinkAll,
      source: path.resolve(ASSETS_DIST_DIR, config.dir_name.voice.main, name),
      target: path.resolve(publicAssetsDir, config.dir_name.voice.main)
    }, {
      fn: file.symlink,
      filename: `${operators[name].fallback_name}.png`,
      source: path.resolve(ASSETS_DIST_DIR, config.dir_name.operator, name),
      ...
      source: path.resolve(ASSETS_DIST_DIR, config.dir_name.charword_table, name),
      target: path.resolve(dataDir)
    }
  ]
  ...
  file.writeSync(JSON.stringify(buildConfig), path.join(path.resolve(dataDir), config.dir_name.config_json))
  file.writeSync(env.generate([
    {
      key: "app_title",
...
export const copyProjectJSON = (name, {
  releaseDir
}) => {
  file.cpSync(
...
export const copyDirectoryData = async ({ dataDir, publicDir }) => {
  ...
  const extractedFolder = path.join(OPERATOR_SOURCE_FOLDER, config.module.operator.directory_assets)
  ...
  const operatorConfig = Object.values(
    Object.values(operators)
      .reduce((acc, cur) => {
        ...
        cur.workshopId = null
        try {
          cur.workshopId = JSON.parse(file.readSync(path.join(ASSETS_DIST_DIR, config.dir_name.project_json, cur.link, config.module.project_json.project_json))).workshopid
        } catch (e) {
  ...
  await Promise.all(config.directory.error.files.map(async (key) => {
    const assetContent = await generateAssetsJson(key.key, extractedFolder)
    file.writeSync(JSON.stringify(assetContent.assetsJson, null), path.join(targetFolder, `${key.key}.json`))
  }))
  ...
  file.writeSync(JSON.stringify(directoryConfig), path.join(dataDir, config.dir_name.config_json))
  ...
  filesToCopy.map((key) => {
    ...
    file.cpSync(path.join(sourceFolder, config.dir_name.operator, key, config.module.operator.assets_json), path.join(targetFolder, `${operators[key].filename}.json`))
    file.cpSync(path.join(sourceFolder, config.dir_name.charword_table, key, config.module.charword_table.charword_table_json), path.join(targetFolder, `voice_${key}.json`))
    file.cpSync(path.join(sourceFolder, config.dir_name.operator, key, portraitName), path.join(targetFolder, portraitName))
  })

  file.cpSync(path.join(extractedFolder, config.directory.error.voice.file), path.join(targetFolder, config.directory.error.voice.target))
}