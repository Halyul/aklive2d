...
import operators, { getOperatorId, OPERATOR_SOURCE_FOLDER } from "@aklive2d/operator";
...
const OPERATOR_IDS = Object.values(operators).map(operator => { return getOperatorId(operator) })
const AUTO_UPDATE_FOLDER = path.resolve(import.meta.dirname, config.dir_name.auto_update)
const CHARWORD_TABLE_FILE = path.resolve(AUTO_UPDATE_FOLDER, config.module.charword_table.charword_table_json)
...
export const lookup = (operatorName) => {
  ...
  return {
    ...
    operator: operatorBlock.ref ? CHARWORD_TABLE.operators[operatorBlock.alternativeId] : operatorBlock,
  }
}

const getDistDir = (name) => {
  return path.join(DIST_DIR, name, config.module.charword_table.charword_table_json)
}

export const getLangs = (name, voiceJson = null) => {
  voiceJson = voiceJson ? voiceJson : JSON.parse(file.readSync(getDistDir(name)))
...
export const build = async (namesToBuild) => {
  ...
  for (const name of names) {
    ...
    const customVoiceName = voiceLangs.filter(i => !config.dir_name.voice.sub.map(e => e.lang).includes(i))[0]
    const voiceLangMapping = config.dir_name.voice.sub.filter(e => {
    ...
    for (const voiceSubFolderMapping of voiceLangMapping) {
      const voiceSubFolder = path.join(OPERATOR_SOURCE_FOLDER, name, config.dir_name.voice.main, voiceSubFolderMapping.name)
      if (file.readdirSync(voiceSubFolder).length === 0) {
        err.push(`Voice folder ${voiceSubFolderMapping.name} for ${name} is empty.`)
      }
    }
  }
...
const updateFn = async (isLocalOnly = false) => {
  ...
  await Promise.all(REGIONS.slice(1).map(async (region) => { await load(region, isLocalOnly) }))
}

const load = async (region, isLocalOnly = false) => {
  const basename = `charword_table_${region}`
  const filename = file.readdirSync(AUTO_UPDATE_FOLDER).filter(item => item.startsWith(`charword_table_${region}`))[0]
  const localFilePath = path.join(AUTO_UPDATE_FOLDER, filename)
  const data = isLocalOnly ? JSON.parse(file.readSync(localFilePath)) : await download(region, path.join(path.dirname(localFilePath), `${basename}.json`))
  ...
  Object.values(data.charWords).forEach(item => {
    const operatorInfo = Object.values(CHARWORD_TABLE.operators).filter(element => element.info[region][item.wordKey])
